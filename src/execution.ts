...
function resolveTimeout(timeoutSeconds: number | ((kwargs: CommandArgs) => number) | undefined, kwargs: CommandArgs): number {
  if (timeoutSeconds === undefined) return DEFAULT_BROWSER_COMMAND_TIMEOUT;
  if (typeof timeoutSeconds === 'function') return timeoutSeconds(kwargs);
...
export async function executeCommand(
  ...
  try {
    if (shouldUseBrowserSession(cmd)) {
      ...
      result = await browserSession(BrowserFactory, async (page) => {
        ...
        try {
          const result = await runWithTimeout(runCommand(cmd, page, kwargs, debug), {
            timeout: resolveTimeout(cmd.timeoutSeconds, kwargs),
            label: fullName(cmd),
          });
      ...
      const timeout = resolveTimeout(cmd.timeoutSeconds, kwargs);
      if (timeout > 0) {
